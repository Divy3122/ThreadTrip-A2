...
struct TripOverviewView: View {
    ...
    private var canvas: Color {
        colorScheme == .dark ? Color(uiColor: .systemGroupedBackground) : Color(red: 0.97, green: 0.96, blue: 0.94)
    }
    ...
    private var tripCard: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            ViewThatFits(in: .horizontal) {
                HStack(spacing: 28) {
                    tripMetric("\(viewModel.trip.durationInDays) days", caption: "Time to explore", symbol: "sun.max")
                    tripMetric(viewModel.tripBudgetText, caption: "Budget per traveller", symbol: "wallet.bifold")
                }
        ...
        .background(LinearGradient(colors: [indigo, Color(red: 0.49, green: 0.28, blue: 0.53)], startPoint: .topLeading, endPoint: .bottomTrailing), in: RoundedRectangle(cornerRadius: 26))
        .shadow(color: indigo.opacity(0.20), radius: 16, y: 8)
    }

    private func tripMetric(_ value: String, caption: String, symbol: String) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            Label(value, systemImage: symbol).font(.headline)
    ...
    private var crewCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                Text("Your travel crew").font(.headline)
                Spacer()
                Text("\(viewModel.trip.activeMembers.count) travellers")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(.secondary)
            }
            LazyVGrid(columns: [GridItem(.adaptive(minimum: textSize.isAccessibilitySize ? 90 : 65))], spacing: 14) {
                ForEach(viewModel.trip.activeMembers) { member in
                    VStack(spacing: 6) {
                        Text(member.initials)
                            ...
                            .background(member.id == viewModel.trip.activeMembers.first?.id ? coral : indigo, in: Circle())
    ...
    private var preferencesCard: some View {
        VStack(alignment: .leading, spacing: 22) {
            VStack(alignment: .leading, spacing: 6) {
                Text("Build your shared deck").font(.title3.bold())
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: textSize.isAccessibilitySize ? 230 : 135))], spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("Maximum estimated cost per traveller")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Slider(value: Binding(get: { viewModel.activityBudget }, set: viewModel.updateActivityBudget), in: 25...300, step: 25)
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Picker("Acceptance rule", selection: Binding(get: { viewModel.trip.decisionPolicy }, set: viewModel.updateDecisionPolicy)) {
            ...
            Label("One locked deck. The same activities, in the same order, for every traveller.", systemImage: "person.3.fill")
                .font(.caption)
                .padding(12)
                .frame(maxWidth: .infinity, alignment: .leading)
                .background(coral.opacity(0.08), in: RoundedRectangle(cornerRadius: 12))
        }
        .padding(20)
        .background(cardColor, in: RoundedRectangle(cornerRadius: 22))
    }
    ...
    private func sectionLabel(_ text: String) -> some View {
        Text(text).font(.caption2.bold()).tracking(0.8).foregroundStyle(.secondary)
    }
    ...
    private func interestChip(_ interest: TravelInterest) -> some View {
        ...
        return Button {
            ...
            .background(selected ? coral : Color(uiColor: .tertiarySystemGroupedBackground), in: RoundedRectangle(cornerRadius: 13))
        }
        .buttonStyle(.plain)
    ...
    private var deckControls: some View {
        VStack(alignment: .leading, spacing: 14) {
            if let deck = viewModel.generatedDeck {
                VStack(alignment: .leading, spacing: 6) {
                    Label("Your shared deck is ready", systemImage: "checkmark.seal.fill")
                        .font(.headline)
                ...
                if let voting = viewModel.voting {
                    NavigationLink {
                        ...
                        actionLabel("Open activity voting", symbol: "hand.draw.fill", color: indigo)
                    }
                    .buttonStyle(.plain)
                }
            } else {
                Button { viewModel.generateSharedDeck() } label: {
                    actionLabel("Generate shared activity deck", symbol: "sparkles", color: coral)
                }
                .buttonStyle(.plain)
            }
    ...
    private func actionLabel(_ title: String, symbol: String, color: Color) -> some View {
        HStack(spacing: 12) {
...
#Preview {
    NavigationStack {
        TripOverviewView(viewModel: TripOverviewViewModel(
            ...
            generateGroupActivityDeck: GenerateGroupActivityDeckUseCase(activityCatalogue: LocalJSONTravelActivityCatalogue())
        ))
    }
}
